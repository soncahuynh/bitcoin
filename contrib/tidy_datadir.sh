#!/bin/bash
# Copyright (c) 2013 The Marscoin Core developers
...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Marscoin database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Marscoin datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Marscoin datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Marscoin 0.7 datadir."
    ;;
  3)
    echo "Detected Marscoin pre-0.8 datadir."
    ;;
  4)
    echo "Detected Marscoin 0.8 datadir."
    ;;
esac
