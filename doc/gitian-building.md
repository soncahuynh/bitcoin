...
*Setup instructions for a Gitian build of Marscoin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Marscoin
...
- [Building Marscoin Core](#building-bitcoin-core)
...
Building Marscoin Core
----------------

To build Marscoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
...
MARSCPATH=/some/root/path/bitcoin
SIGPATH=/some/root/path/bitcoin-detached-sigs

./bin/gbuild --url bitcoin=${MARSCPATH},signature=${SIGPATH} ../bitcoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
